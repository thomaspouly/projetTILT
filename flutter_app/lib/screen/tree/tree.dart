...
class _TreePageState extends State<TreePage> {
  ...
  prepareAnimation() async {
    ...
    var emojiComposition = await instance
        .loadAnimationFromAsset("assets/animations/pot.json");
  ...
  Widget build(BuildContext context) {
    ...
    Widget content =  ready ? buildStarContent(context) : Text("Loading animation...");
    ...
    return new SafeArea(
        child: new Scaffold(
          ...
          appBar: AppBar(
            ...
            body: Stack(
      children: <Widget>[
      Center(child:Container(padding: EdgeInsets.only(bottom: 50),width:300,height:400,child: content,)),
       
        Align(
          
          alignment: Alignment.bottomCenter,
          child: Container(padding: EdgeInsets.only(bottom: 25),child:RaisedButton(
            elevation: 5,
            color: Colors.blue,
            onPressed: () {
              _showForm();
            },
            child: Text("Formulaire", style: TextStyle(fontSize: 20,color: Colors.white)),
          ),),
        ),
       /* Align(
